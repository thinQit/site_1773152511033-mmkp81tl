...
export default function GalleryMasonry({
  headline = 'Salon Transformations Gallery',
  subheadline = 'A glimpse into our signature hair, makeup, and skincare artistry.',
  images = [],
      ...
      <div className="container mx-auto max-w-7xl px-4">
          ...
          {images.map(function (img, i) {
            return (
              <div key={i} className="group relative aspect-square cursor-pointer overflow-hidden rounded-xl" onClick={function () { setSelectedImage(img); }}>
                <Image src={img.url} alt={img.alt} width={700} height={700} unoptimized className="h-full w-full object-cover transition-transform duration-300 group-hover:scale-105" />
        ...
        {selectedImage && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4" onClick={function () { setSelectedImage(null); }}>
            <Image src={selectedImage.url} alt={selectedImage.alt} width={1200} height={900} unoptimized className="max-h-[85vh] max-w-[90vw] rounded-lg object-contain" />
          </div>
        )}
      </div>
    </section>
